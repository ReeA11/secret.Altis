...
switch _mode do {
			...
			case "weapons": {
				{
					...
					_price1 = [_price,0] call UnionClient_system_discount;
					if (call compile _condition) then {
						...
						_ctrlTV tvSetData [[_weaponIndex], str ([_class, _price1, _display])];
			...
			default {
				{
					...
					if (call compile _condition) then {
						...
						_ctrlTV tvSetPicture [[_weaponIndex], _itemPicture];
						_ctrlTV tvSetData [[_weaponIndex], str ([_class, _price1, _display])];
						_ctrlTV tvSetValue [[_weaponIndex], _price1];
					};
				} forEach (getArray (MAVV(Cfg) >> _cat));
			};
		};
    ...
    case "onTreeChanged": {
		...
		_price1 = [_price,0] call UnionClient_system_discount;
		private _afford = (MAVV(Total) + _price1) > CASH;
    ...
    case "add": {
		...
		_cartList lbSetValue [_index, _price1];
		_cartList lbSetPicture [_index, _itemPicture];

		MAVV(Cart) pushBack [toLower _className, _price1];
		MAVV(Total) = MAVV(Total) + _price1;
	...
	case "complete": {
		...
		_cash = [_cash,0] call UnionClient_system_discount;
		["cash","take",_cash] call UnionClient_system_myCash;
