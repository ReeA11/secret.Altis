...
[] spawn {
	...
	_positions = getArray(missionConfigFile >> "Maverick_Shipwrecks1" >> "Positions");
	_ship = objNull;
	_ship2 = objNull;
	_lastCreateTime = time - 360;
	_radius = getNumber(missionConfigFile >> "Maverick_Shipwrecks1" >> "Radius");
	...
	_createNewWreck = {
		...
		_marker setMarkerText "Крушение";
		...
		} forEach selectRandom(getArray(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipLoot"));
		...
		getText(missionConfigFile >> "Maverick_Shipwrecks1" >> "SpawnMSG") remoteExecCall ["hint",0];
	};

	_deleteOldWreck = {
		if(!isNull _ship) then {
			getText(missionConfigFile >> "Maverick_Shipwrecks1" >> "DeleteMSG") remoteExecCall ["hint",0];
			sleep 5;
			deleteVehicle _ship;
	...
	while {true} do {
		if(isNull _ship) then {
			if((time - _lastCreateTime) >= (getNumber(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipwreckInterval"))) then {
				[] call _deleteOldWreck;
				[] call _createNewWreck;
				_lastCreateTime = time + (getNumber(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipwreckInterval"));
			};
		} else {
