...
if( hasInterface ) exitWith 
{
	...
	player addAction[getText(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipLootText"),
	{
        [] spawn {
            ...
            {
                if([true,_x,1] call UnionClient_system_handleInv) then {
                    _index = _forEachIndex;
                } else {
                    _allGood = false;
                };
                if(!_allGood) exitWith {hint getText(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipLootFail");};
            } forEach _lootArray;
            if(_allGood) then {
                hint getText(missionConfigFile >> "Maverick_Shipwrecks1" >> "ShipLootSuccess");
