/*
   ...
   заметить в mpmission.altis>>code>>shops
   52-54  костыль на спавн по z;
...
if((_spawnPoints select 0) == "cop_air_1") then {
_sp = [(_sp select 0),(_sp select 1),0.84];
};
if((_spawnPoints select 0) == "cop_car_1") then {
_sp = [(_sp select 0),(_sp select 1),4.390];
};
if((_spawnPoints select 0) == "g_car_bst") then {
_sp = [(_sp select 0),(_sp select 1),1.312];
};
if((_spawnPoints select 0) == "g_air_bst") then {
_sp = [(_sp select 0),(_sp select 1),1.45];
};
