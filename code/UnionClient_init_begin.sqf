...
"Starting Verum Client..." call UnionClient_system_log;
...
[] spawn {
	while {true} do{
		...
		(findDisplay 49) displaySetEventHandler ["KeyDown", "_this call ESCfunc;"];
		sleep 1;
	};
};
...
[] spawn {
	while {true} do{
		waitUntil {!isnull (findDisplay 153)};
		(findDisplay 153) displaySetEventHandler ["KeyDown", "_this call ESCfunc;"];
		sleep 1;
	};
};
...
[] spawn {
	while {true} do{
		...
		if(_backpack==_backpack_x) then {
			...
			hint "Дюпать плохо, особенно на нашем проекте...";
...
format["End of Verum Client Init :: Total Execution Time %1 seconds",(diag_tickTime - _timeStamp)] call UnionClient_system_log;