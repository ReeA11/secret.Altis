...
if !([_item] call UnionClient_system_canUseItem4) then {
	[_item] call UnionClient_system_removeItem;
	hint "Вы не можете использовать этот предмет";
};

if (EQUAL(_item,"sab_CUAV_Terminal") && !license_civ_press && (life_coplevel < 24)) then {
	[_item] call UnionClient_system_removeItem;
	hint "Беспилотники доступны только для прессы и полиции высокого ранга";
};
